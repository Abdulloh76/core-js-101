...
function isInsideCircle(circle, point) {
  const distance = Math.sqrt(
    Math.abs((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2),
  );
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const smallerOne = a > b ? b : a;
  const biggerOne = a > b ? a : b;
  return `${isStartIncluded ? '[' : '('}${smallerOne}, ${biggerOne}${isEndIncluded ? ']' : ')'}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < configs.length; i += 1) {
    if (brackets.includes(configs[i])) {
      brackets = brackets.replace(configs[i], '');
      i = -1;
    }
  }
  return !(brackets);
...
function toNaryString(num, n) {
  ...
  if (result === 0) return (num % n).toString();
  return toNaryString(result, n) + (num % n).toString();
}
...
function evaluateTicTacToePosition(position) {
  // position[i][j] === position[i][j-1] along rows
  ...
  // position[j][i] === position[j-1][i] along columns
  ...
  // along diagonal position[i][i] === position[i-1][i-1]
  if (position[0][0] === position[1][1] && position[1][1] === position[2][2]) {
    return position[2][2];
  }
  // along opposite diagonal position[i-1][3-i] === position[i][2-i]
